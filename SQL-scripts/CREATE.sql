...
CREATE UNIQUE INDEX destino_id_dest ON destino (id_dest);
CREATE UNIQUE INDEX destino_codigo ON destino (codigo);
...
ALTER TABLE destino ADD CONSTRAINT FK_TIDE_DEST FOREIGN KEY (id_tide) REFERENCES tipo_destino (id_tide);
ALTER TABLE destino ADD CONSTRAINT FK_DECA_DEST FOREIGN KEY (id_deca) REFERENCES DESTINATION_CATEGORIES (id_deca);
